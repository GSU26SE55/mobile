...
import {
  CommentThread,
  type ChatTab,
} from "../../tickets/components/CommentThread";
...
import {
  ...
  isTicketChatLocked,
  shouldShowLiveSla,
} from "../../tickets/utils/ticketWorkflow";
...
import {
  useAddReaction,
  useRemoveReaction,
} from "../../tickets/hooks/useChatReactions";
...
function BubbleChat({
  ...
  useEffect(() => {
    if (notificationId)
      void notificationService.markOpened(notificationId).catch(() => {});
  }, [notificationId]);
  ...
  const customerDetailQuery = useTicketDetail(
    !isOperator && activeTicketId ? activeTicketId : "",
  );
  const staffDetailQuery = useStaffTicketDetail(
    isOperator && activeTicketId ? activeTicketId : "",
  );

  const ticketDetail = isOperator
    ? staffDetailQuery.data
    : customerDetailQuery.data;
  const ticketLoading = isOperator
  ...
  const send = useCallback(
    (
      ticketId: string,
      payload: Parameters<typeof staffTicketService.addComment>[1],
    ) =>
      isOperator
  ...
  const kbRefsQuery = useTicketKbRefs(
    isOperator && activeTicketId ? activeTicketId : undefined,
  );
  ...
  const comments = useMemo(() => {
    ...
    return (commentsQuery.data?.pages ?? [])
      ...
      .filter((comment) => {
        if ((!isOperator && comment.isInternal) || seen.has(comment.id))
          return false;
        seen.add(comment.id);
        return true;
      });
  ...
  const publicCount = useMemo(
    () => comments.filter((c) => !c.isInternal).length,
    [comments],
  );
  ...
  const pinningId = pinChat.isPending
    ...
    : unpinChat.isPending
      ...
      : null;
  const voiceLevel =
    voiceRecorder.waveform.reduce((sum, value) => sum + value, 0) /
  ...
  const handleCustomerPickAttachment = async () => {
    ...
    if (!permission.granted) {
      Alert.alert(
        "Permission Required",
        "Photo library access is required to attach a file.",
      );
      return;
    }
  ...
  if (!activeTicketId) {
    ...
    const isListLoading = isOperator
      ? staffTicketsQuery.isLoading
      : customerTicketsQuery.isLoading;
  ...
  const operatorSendDisabled =
    customerSendDisabled || operatorUploading || voiceRecorder.isRecording;

  return (
    <View style={styles.root}>
      {/* Top Messenger Single-Row Compact Header Bar */}
      <View
        style={[styles.headerCompact, { paddingTop: Math.max(insets.top, 8) }]}
      >
        ...
        {isOperator && viewMode === "chat" && (
          <View style={styles.headerTabSwitchInline}>
            <Pressable
              style={[
                styles.inlineSwitchBtn,
                chatTab === "public" && styles.inlineSwitchBtnActivePublic,
              ]}
            ...
            >
              ...
              <Text
                style={[
                  ...
                  chatTab === "public" && styles.inlineSwitchTextActivePublic,
                ]}
              >
                {publicCount}
              </Text>
            ...
            <Pressable
              style={[
                styles.inlineSwitchBtn,
                chatTab === "internal" && styles.inlineSwitchBtnActiveInternal,
              ]}
            ...
            >
              <Ionicons
                ...
                size={10}
                color={
                  chatTab === "internal" ? Colors.warning : Colors.textMute
                }
              />
              <Text
                style={[
                  styles.inlineSwitchText,
                  chatTab === "internal" &&
                    styles.inlineSwitchTextActiveInternal,
                ]}
              >
                {internalCount}
              </Text>
        ...
        <Animated.View style={[{ flex: 1 }, chatContainerAnimatedStyle]}>
          <CommentThread
            ...
            onDelete={(comment, reason) =>
              deleteChat.mutate({ chatId: comment.id, reason })
            }
            editPending={updateChat.isPending}
            deletePending={deleteChat.isPending}
            onMarkRead={(chatIds, onFailed) =>
              markChatsRead.mutate({ chatIds, onFailed })
            }
            onTranslate={async (comment, targetLanguage) => {
              const response = await translateChat.mutateAsync({
                chatId: comment.id,
                targetLanguage,
              });
            ...
            onPin={
              isOperator ? (comment) => pinChat.mutate(comment.id) : undefined
            }
            onUnpin={
              isOperator ? (comment) => unpinChat.mutate(comment.id) : undefined
            }
            ...
            onDownloadAttachments={(comment, fileIds) =>
              downloadAttachments(comment.id, fileIds)
            }
          ...
          {!chatLocked && (
            <TypingIndicator
              names={typingUsers.map((tUser) => tUser.displayName)}
            />
          )}
          ...
          {chatLocked ? null : isOperator ? (
            <Animated.View
              style={[styles.operatorComposer, composerAnimatedStyle]}
            >
              <AttachmentPreviewStrip
                ...
                onRemove={(fileId) =>
                  setAttachments((items) =>
                    items.filter((item) => item.fileId !== fileId),
                  )
                }
              />
              <View style={styles.composerRow}>
                ...
                <Pressable
                  style={[
                    styles.sendBtn,
                    operatorSendDisabled && styles.disabled,
                  ]}
                  onPress={handleSend}
            ...
            <Animated.View style={[styles.composer, composerAnimatedStyle]}>
              ...
              <Pressable
                style={[
                  styles.sendBtn,
                  customerSendDisabled && styles.disabled,
                ]}
                onPress={handleSend}
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={styles.subTabsBar}>
            <Pressable
              style={[
                styles.subTabItem,
                detailSubTab === "details" && styles.subTabItemActive,
              ]}
            ...
            >
              <Text
                style={[
                  styles.subTabLabel,
                  detailSubTab === "details" && styles.subTabLabelActive,
                ]}
              >
                Details
              </Text>
            ...
            <Pressable
              style={[
                styles.subTabItem,
                detailSubTab === "activities" && styles.subTabItemActive,
              ]}
            ...
            >
              <Text
                style={[
                  styles.subTabLabel,
                  detailSubTab === "activities" && styles.subTabLabelActive,
                ]}
              >
                History
              </Text>
            ...
            {isOperator && (
              <Pressable
                style={[
                  styles.subTabItem,
                  detailSubTab === "logs" && styles.subTabItemActive,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.subTabLabel,
                    detailSubTab === "logs" && styles.subTabLabelActive,
                  ]}
                >
                  Logs
                </Text>
            ...
            {isOperator && (
              <Pressable
                style={[
                  styles.subTabItem,
                  detailSubTab === "kb" && styles.subTabItemActive,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.subTabLabel,
                    detailSubTab === "kb" && styles.subTabLabelActive,
                  ]}
                >
                  Guide
                </Text>
                ...
                <>
                  ...
                  <View style={[styles.detailCard, Shadow]}>
                    <View style={styles.cardHeader}>
                      ...
                      {ticketDetail.resolutionSlaTimer &&
                        shouldShowLiveSla(
                          ticketDetail.status,
                          ticketDetail.priority,
                          ticketDetail.resolutionSlaTimer.status,
                        ) && (
                          <SlaCountdown sla={ticketDetail.resolutionSlaTimer} />
                        )}
                    </View>
                  ...
                  {isOperator ? (
                    <View style={{ marginTop: 8 }}>
                      <TicketActionBar
                        ...
                        onResume={() =>
                          resumeTicket.mutate({
                            reason: "Blocking condition cleared.",
                          })
                        }
                        ...
                        canResolve={
                          checkPermission(user, P.TICKET_RESOLVE) &&
                          canComplete(ticketDetail, accountId)
                        }
          ...
          {detailSubTab === "logs" && isOperator && (
            <ScrollView contentContainerStyle={styles.detailsScroll}>
              {ticketDetail?.status === "InProgress" &&
                isPrimaryHandler(ticketDetail, accountId) && (
                  <Pressable
              ...
              {ticketDetail?.maintenanceLogs &&
              ticketDetail.maintenanceLogs.length > 0 ? (
                ticketDetail.maintenanceLogs.map((log) => (
                  <View
                    key={log.id}
                    style={[styles.detailCard, Shadow, { marginTop: 10 }]}
                  >
                    <View style={styles.cardHeader}>
                      <Text style={styles.logTitle}>
                        {log.summary || "Maintenance Log"}
                      </Text>
              ...
              {kbRefsQuery.data && kbRefsQuery.data.length > 0 ? (
                kbRefsQuery.data.map((ref) => (
                  ...
                  >
                    <View style={styles.cardHeader}>
                      <Text style={styles.logTitle}>
                        {ref.kbArticleTitle || ref.kbArticleCode}
                      </Text>
                    ...
                    <Text style={styles.metaLabel}>
                      Article code: {ref.kbArticleCode}
                    </Text>
                  </View>
                ))
              ) : (
                <Text style={[styles.emptyText, { marginTop: 20 }]}>
                  No referenced guide articles yet.
                </Text>
              )}
            </ScrollView>
          )}
                  ...
                  initialValues={
                    editingLog
                      ? {
                          ...
                          diagnosisDetails:
                            editingLog.diagnosisDetails ?? undefined,
                          actionsTaken: editingLog.actionsTaken ?? undefined,
                          resolutionNote:
                            editingLog.resolutionNote ?? undefined,
