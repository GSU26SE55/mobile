...
export function KbReferencePicker({ visible, ticketId, onClose }: Props) {
  ...
  const { data: suggestions = [], isLoading } = useKbSuggest(
    visible ? ticketId : undefined,
  );
  const { mutate: addRef } = useAddKbRef(ticketId);
  const [refType, setRefType] = useState<KbReferenceTypeEnum>(
    KbReferenceTypeEnum.ConsultedDuringResolve,
  );
  ...
  const handleAssign = (kbArticleId: string) => {
    ...
    addRef(
      { ticketId, kbArticleId, referenceType: refType },
      {
        onSuccess: () => {
          setPendingId(null);
          setRefType(KbReferenceTypeEnum.ConsultedDuringResolve);
          onClose();
        },
            ...
            {REF_TYPES.map((t) => (
              ...
              >
                <Text
                  style={[
                    styles.typeChipText,
                    refType === t && styles.typeChipTextOn,
                  ]}
                >
