 ...
 */
function parseSensorReading(notes: string): { sensor: string; measured: string; limit: string } | null {
  const m = notes.match(/^(.+?)\s+raw=([\d.]+)\s*>\s*thr=([\d.]+)/i);
  if (!m) return null;
  return { sensor: m[1].trim(), measured: m[2], limit: m[3] };
}

/** Settled (resolved / false alarm) vs still needs someone on site. Colouring by that split
 *  rather than by severity is deliberate — every incident that reaches a ticket is Critical, so
 *  a severity-driven badge would be red on all of them and carry no signal. */
function isSettled(status: EnvironmentalIncidentStatusEnum) {
  return (
 ...
 */
export default function EnvironmentalIncidentCard({ incidentId, description }: Props) {
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          <Text style={styles.title} numberOfLines={1}>
            {incident ? INCIDENT_TYPE_LABEL[incident.incidentType] ?? 'Environmental incident' : 'Environmental incident'}
          </Text>
        ...
        {incident && (
          <View
            style={[
              styles.badge,
              { backgroundColor: isSettled(incident.status) ? Colors.card2 : Colors.dangerLight },
            ]}
          >
            <Text
              style={[
                styles.badgeText,
                { color: isSettled(incident.status) ? Colors.textMute : Colors.dangerDark },
              ]}
            >
      ...
      {evidence && (
        <View style={styles.evidenceWrap}>
            ...
            <View style={styles.breachBox}>
              <View style={styles.breachRow}>
                ...
                <Text style={styles.breachValue}>
                  ...
                  <Text style={styles.breachLimit}>{` > ${reading.limit}`}</Text>
                </Text>
              </View>
          ...
          <Text style={styles.footNote}>
            The fault is in the cabinet, not in one battery, so there is no battery reading log to
            cross-check.
          </Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  sub: { fontSize: 11, color: Colors.textMute, marginTop: 3, fontWeight: '600' },
  ...
  breachRow: { flexDirection: 'row', alignItems: 'baseline', justifyContent: 'space-between', gap: 12 },
  breachSensor: { fontSize: 12, color: Colors.textMute, fontWeight: '600', flexShrink: 1 },
  breachValue: { fontSize: 12 },
  breachMeasured: { fontWeight: '800', color: Colors.danger },
  breachLimit: { color: Colors.textMute },
  breachNote: { fontSize: 11, color: Colors.textMute, marginTop: 6, fontWeight: '500' },
  rawEvidence: {
    fontSize: 11,
    color: Colors.text,
  ...
  footNote: { fontSize: 11, color: Colors.textMute, marginTop: 6, fontWeight: '500' },
});
