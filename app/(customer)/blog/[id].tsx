...
export default function CustomerBlogDetailScreen() {
  const insets = useSafeAreaInsets();
  const { id } = useLocalSearchParams<{ id: string }>();
  const { data, isLoading, isError, error, refetch } = useBlogDetail(id);

  // BE returns 404 when the post is not in Published status (Draft/Archived/removed).
  // Other errors (network loss, 5xx) do NOT mean "post removed" — must allow retry.
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      {isLoading ? (
        ...
        // WebView scrolls its own content → the title block stays fixed, not wrapped in ScrollView.
